...
function Minigame2Page() {
  ...
  const chooseNewWord = useCallback(() => {
    ...
    const letters = word.split('').map(char => ({ char, typed: false }));
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      setCurrentWord(prevWord => {
        ...
        if (firstUntypedIndex !== -1 && newWord[firstUntypedIndex].char === typedChar) {
          newWord[firstUntypedIndex].typed = true;
          typedCorrectly = true;
        }
  ...
  useEffect(() => {
    if (currentWord.length > 0 && currentWord.every(letter => letter.typed)) {
      ...
      if (sparkAudioRef.current) {
        sparkAudioRef.current.currentTime = 0;
        sparkAudioRef.current.play();
      }
      setWordFade(true);
      setTimeout(() => {
        chooseNewWord();
      }, 400); // Corresponds to fadeOut animation duration
    }
  }, [currentWord, chooseNewWord]);

  return (
    <div className="container-fluid p-0 minigame2-container">
      <div className="wrapper">
        <h1>Typing Minigame</h1>
        ...
        <div className="wordsWrap">
          <div ref={wordRef} className={`words ${wordFade ? 'animated fadeOut' : ''}`}>
