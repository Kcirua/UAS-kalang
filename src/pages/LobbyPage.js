...
function LobbyPage() {
  ...
  const avatars = useMemo(() => [grayAvatar, greenAvatar, redAvatar], []);
  const avatarNames = useMemo(() => ['Gray Frog', 'Green Frog', 'Red Frog'], []);
  const avatarDescriptions = useMemo(() => [
  ...
  const handleStartClick = () => {
    ...
    // Logika navigasi setelah animasi singkat, tanpa menyimpan ke localStorage
    setTimeout(() => {
      navigate('/main', { state: { playerName: playerName } });
    }, 500);
  };
